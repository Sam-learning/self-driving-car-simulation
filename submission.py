...
class ExactInference(object):
    ...
    #目的:update belief matrix based on sensor data
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE
        # belief matrix dimension = environment grid dimension. belief matrix是機率，代表其他車子出現在each cell的機率，初始化用uniform
        # iterate though all cells(代表環境的一小格) 每個cell都計算
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                x = util.colToX(col)
                y = util.rowToY(row)

                #agent代表self-driving car 的位置資訊
                #likelihood為其他車子在該格子cell的機率
                #mean設為dist，dist是agent到這個iteration的cell的距離
                #observedDist : agent sensor 觀察到的有關其他車輛的資訊
                prior = self.belief.getProb(row, col)
                dist = math.sqrt((agentX - x)**2 + (agentY - y)**2)
                likelihood = util.pdf(dist, Const.SONAR_STD, observedDist)  #mean, std, value
                
                #likelihood*prior = postirior probability
                self.belief.setProb(row, col, likelihood * prior)
    ...
    ##################################################################################
    #先call observe() update the belief state based on sensor, then call elapseTime() update the belief matrix base on motion of the car.
    def elapseTime(self) -> None:
        ...
        #oldTile:index of cell other car at in previous time step
        #newTile:index of neighboring cell that the other car can moved to in current time step.
        for (oldTile, newTile) in self.transProb:
            prior = self.belief.getProb(oldTile[0], oldTile[1])
            transition = self.transProb[(oldTile, newTile)]
            posterior = prior * transition               #代表轉換造成的機率改變
            newBelief.addProb(newTile[0], newTile[1], posterior)
        newBelief.normalize()                 #delta normalize之後就是posterior probability
    ...
    def elapseTime(self) -> None:
        ...
        for old_tile in self.particles:
            for _ in range(self.particles[old_tile]):
                new_tile = util.weightedRandomChoice(self.transProbDict[old_tile])
